...
export class PricingService {
  ...
  /**
   * Get pricing for multiple properties and calculate total prices (legacy method)
   */
  async getPropertiesWithPricing(
    properties: Property[],
    startDate: string,
    endDate: string
  ): Promise<PropertyWithPricing[]> {
    ...
    // Get pricing for all properties
    const pricingMap = await this.ownerRezService.getPropertiesPricing(
      propertyIds,
      startDate,
      endDate
    );

    // Combine properties with their pricing data
    const propertiesWithPricing: PropertyWithPricing[] = properties.map(property => {
      const pricingData = pricingMap.get(property.id) || [];
      const totalPrice = this.calculateTotalPrice(pricingData);
   ...
   */
  filterByPriceRange(
    properties: PropertyWithPricing[],
    minPrice?: number,
    maxPrice?: number
  ): PropertyWithPricing[] {
