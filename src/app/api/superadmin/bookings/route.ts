...
async function fetchAllBookings(limit: number = 50, offset: number = 0, sinceDate?: string, status?: string) {
  ...
  const url = `${v2Url}/bookings?limit=${limit}&offset=${offset}${sinceParam}${statusParam}`;
  
  if (process.env.NODE_ENV !== 'production') {
    console.log('Superadmin Bookings API Request:', {
      status: status,
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log('Superadmin Bookings API Response:', {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log('Superadmin Bookings data received meta:', {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `superadmin-limit=${limit}&offset=${offset}&since=${sinceDate}${statusKey}`;
    const cached = bookingsCache.get(cacheKey);
    if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
      return NextResponse.json(cached.payload, { headers: { 'X-Cache': 'HIT' } });
    }

    // Fetch all bookings (no property filtering for superadmin)
    const bookingsData = await fetchAllBookings(limit, offset, sinceDate, status || undefined);

    if (process.env.NODE_ENV !== 'production') {
      console.log("Superadmin bookings-meta", { 
    ...
    console.error('Error fetching superadmin bookings:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch superadmin bookings' },
      { status: 500 }
    );
  }
}
