...
async function fetchAdminBookings(limit: number = 50, offset: number = 0, sinceDate?: string, propertyIds: number[] = [], status?: string) {
  if (!username || !password || !v2Url) {
    throw new Error('API credentials not configured');
  }

  if (!propertyIds || propertyIds.length === 0) {
    // If admin has no properties, return empty result
    return {
      items: [],
      total: 0,
      limit,
  ...
  const propertyParam = `&property_ids=${propertyIds.join(',')}`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get admin's property IDs
    let propertyIds: number[] = [];
    if (authResult.user.role === 'admin' && email) {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.log('Admin properties found:', { email, propertyIds });
        }
      } catch (e) {
    ...
    } else if (authResult.user.role === 'superadmin') {
      // Superadmin can see all bookings, so we don't filter by property IDs
      propertyIds = [];
    }
    ...
    const cacheKey = `admin-${email}-limit=${limit}&offset=${offset}&since=${sinceDate}${pidKey}${statusKey}`;
    const cached = bookingsCache.get(cacheKey);
    if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
      return NextResponse.json(cached.payload, { headers: { 'X-Cache': 'HIT' } });
    }

    // Fetch bookings with property filtering
    const bookingsData = await fetchAdminBookings(limit, offset, sinceDate, propertyIds, status || undefined);

    if (process.env.NODE_ENV !== 'production') {
      console.log("Admin bookings-meta", { 
        items: Array.isArray(bookingsData?.items) ? bookingsData.items.length : 0, 
        role: authResult.user.role, 
        filtered: Array.isArray(propertyIds) && propertyIds.length > 0,
        propertyIds: propertyIds
      });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch admin bookings' },
      { status: 500 }
    );
  }
}