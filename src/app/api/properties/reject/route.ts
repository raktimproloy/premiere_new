...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only superadmin can reject properties
    if (authResult.user.role !== 'superadmin') {
      return NextResponse.json(
        { success: false, message: 'Insufficient permissions. Only superadmins can reject properties.' },
        { status: 403 }
      );
    }
    ...
    console.log('Attempting to reject property with ID:', propertyId);
    ...
    // Update property in OwnerRez to set active = false
    ...
    if (!ownerRezRes.ok) {
      ...
      return NextResponse.json(
        { 
          success: false, 
          message: 'Failed to reject property in OwnerRez',
          details: errorData,
          ownerRezId: property.ownerRezId
        },
    ...
    const updateResult = await db.collection("properties").updateOne(
      ...
      { 
        $set: { 
          status: 'rejected',
          updatedAt: new Date(),
          rejectedBy: authResult.user._id,
          rejectedAt: new Date(),
    ...
    console.error('Property rejection API error:', error);
